...
A caution learned the hard way: notice titles come back in all 24 official EU
languages, but what TED translates is the "Country - CPV label - " prefix it puts
in front. The buyer's own project title is left in the original language, so once
that prefix is stripped the 24 versions collapse to one identical string. We keep
only genuinely different wordings, and label a title with the language it is
actually written in - not with the language key it arrived under.
...
def _post(payload, session, log=None):
    """TED answers 429 with an HTML body rather than JSON; the shared client
    handles the waiting and raises if it really cannot get through."""
    return http_client.post_json(session, ENDPOINT, payload, log=log)
...
def to_record(notice):
    ...
    title, title_key = _pick_language(titles_raw)
    ...
    for lang, text in titles_raw.items():
        ...
        stripped[code] = f" {DASH} ".join(parts[2:]).strip() if len(parts) >= 3 else text

    description, description_key = _pick_language(notice.get("description-proc") or {})
    description_lang = record.lang2(description_key)

    # The title is written in whatever language the buyer used, which is the
    # language of the description, not the key the title arrived under. Keep only
    # the variants that genuinely differ, so we never claim a translation we
    # do not have.
    title_lang = description_lang or record.lang2(title_key)
    titles = {title_lang: title} if (title_lang and title) else {}
    titles.update({code: text for code, text in stripped.items() if text and text != title})
...
def fetch(since_days=2, limit=None, log=print):
    ...
    while True:
        ...
        data = _post(payload, session, log)
